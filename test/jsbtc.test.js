...
describe(`${(browser) ? 'Browser' : 'Node'} test jsbtc library`, function () {
    ...
    describe("Hash functions:", function () {
        ...
        it('siphash', () => {
            ...
            assert.equal(jsbtc.siphash(jsbtc.tools.stringToBytes("siphash test")).toString('hex'),
                "84c247dc01541d54");
            expect(() => jsbtc.siphash("", {
                v0: 43,
                v1: v1
            }).toString('hex')).to.throw('siphash init vectors v0, v1 must be BN instance');
            expect(() => jsbtc.siphash("", {
                v0: v0,
                v1: 11
            }).toString('hex')).to.throw('siphash init vectors v0, v1 must be BN instance');
    ...
    describe("Address functions:", function () {
        it('hashToAddress', () => {
            ...
            assert.equal(jsbtc.hashToAddress(script_hash.toString('hex'), {
                    testnet: false,
                    script_hash: true,
                    witness_version: null
                }),
        ...
        it('publicKeyToAddress', () => {
            ...
            assert.equal(jsbtc.publicKeyToAddress(cpub, {
                witness_version: null,
                testnet: true
            }), "mvNyptwisQTmwL3vN8VMaVUrA3swVCX83c");
            let priv = "L32a8Mo1LgvjrVDbzcc3NkuUfkpoLsf2Y2oEWkV4t1KpQdFzuyff";
            assert.equal(jsbtc.publicKeyToAddress(jsbtc.privateToPublicKey(priv), {
                p2sh_p2wpkh: true,
                witness_version: null
            }), "33am12q3Bncnn3BfvLYHczyv23Sq2Wbwjw");
            expect(() => jsbtc.publicKeyToAddress(jsbtc.privateToPublicKey(priv, {compressed: false}), {p2sh_p2wpkh: true})).to.throw('length invalid');
            expect(() => jsbtc.publicKeyToAddress(jsbtc.privateToPublicKey(priv, {compressed: false}), {
                p2sh_p2wpkh: false,
                witness_version: 0
    ...
    describe("Transactions classes:", function () {
        it('Transaction', () => {
            ...
            let tx = new jsbtc.Transaction({raw_tx: t});
            for (let q in j) {
                if (tx[q] instanceof Object) continue;
                assert.equal(tx[q], j[q]);
            }

            for (let q in j.vIn) for (let z in j.vIn[q]) assert.equal(tx.vIn[q][z], j.vIn[q][z]);
            for (let q in j.vOut) for (let z in j.vOut[q]) assert.equal(tx.vOut[q][z], j.vOut[q][z]);
            ...
            tx = new jsbtc.Transaction({raw_tx: t});
            tx.encode();
            tx.decode();
            for (let q in j) {
                if (tx[q] instanceof Object) continue;
            ...
            for (let q in j.vIn) for (let z in j.vIn[q]) {
                if (tx.vIn[q][z] instanceof Array) {
                    for (let p in tx.vIn[q][z]) {
                        assert.equal(tx.vIn[q][z][p], j.vIn[q][z][p])
                    }
                } else assert.equal(tx.vIn[q][z], j.vIn[q][z]);
            }
            for (let q in j.vOut) for (let z in j.vOut[q]) assert.equal(tx.vOut[q][z], j.vOut[q][z]);

            assert.equal(tx.serialize(), t);
        ...
        it('Download raw block mainnet 520667 size: 1,160,482 bytes - https://bitaps.com/520667', async () => {
            ...
            let b = await r.text();
            rawBlock = jsbtc.Buffer.from(b, 'hex');
        }).timeout(60000);

        it('Deserialize block[520667] 2 592 transactions in raw format', () => {
            rawBlock.seek(80);
            let c = jsbtc.tools.varIntToInt(rawBlock.readVarInt());
            for (let i=0; i<c; i++) new jsbtc.Transaction({raw_tx: rawBlock, format: 'raw'});
        });

        it('Deserialize block[520667]  2 592 transactions in decoded human readable format', () => {
            rawBlock.seek(80);
            let c = jsbtc.tools.varIntToInt(rawBlock.readVarInt());
            for (let i=0; i<c; i++) new jsbtc.Transaction({raw_tx: rawBlock, format: 'decoded'});
        });
        ...
        it('Deserialize <-> serialize test block[520667]  2 592 transactions decoded', () => {
            ...
            let c = jsbtc.tools.varIntToInt(rawBlock.readVarInt());
            for (let i=0; i<c; i++) {
                let t = new jsbtc.Transaction({raw_tx: rawBlock, format: 'decoded', keep_raw_tx:true});
                assert.equal(t.serialize(), t.rawTx);
            }
        });
        ...
        it('Deserialize <-> serialize test block[520667]  2 592 transactions raw', () => {
            ...
            let c = jsbtc.tools.varIntToInt(rawBlock.readVarInt());
            for (let i=0; i<c; i++) {
                let t = new jsbtc.Transaction({raw_tx: rawBlock, format: 'raw', keep_raw_tx:true});
                assert.equal(t.serialize(), t.rawTx.hex());
            }
        });
        ...
        it('--', () => {
            ...
            let tx = new jsbtc.Transaction({raw_tx: t, format: 'raw'});
            console.log(tx.json());
        });
